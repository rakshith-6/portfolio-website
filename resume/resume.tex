...
\begin{document}
	...
	\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}} l r}
		...
		{\color{accent}\small Robotics \& Autonomous Systems $\cdot$ AI\&ML} &
		\small +91 6363759090 \\
		& \small \href{https://github.com/rakshith-6}{GitHub} $|$  \href{https://www.linkedin.com/in/rakshith-ajetaravi}{LinkedIn} \\
	\end{tabular*}
	\vspace{2pt}
	\hrule height 0.6pt
	\vspace{-2pt}
	...
	\begin{itemize}[leftmargin=0.15in, label={}]
		...
		\small{\item{
				...
				\textbf{Control}{: LQR, MPC, Sliding Mode Control, Reinforcement Learning Based Control} \\
	...
	\begin{itemize}[leftmargin=0.15in, label={}]
		...
		\vspace{-6pt}
		\resumeItemDesc{A multimodal \textbf{Retrieval-Augmented Generation (RAG)} tutor that lets users query educational documents and videos through an interactive \textbf{Gradio} interface and CLI, give answers with exact page and timestamp citations. The pipeline extracts audio via \textbf{Whisper} (faster-whisper), detects scene changes and captions video frames with a vision-language model (\textbf{Qwen2.5-VL}), and extracts structured text from PDFs/DOCX, generating embeddings with \textbf{BGE-M3} stored in \textbf{ChromaDB}. Implemented hybrid retrieval with cross-encoder reranking and a self-correcting LangGraph agent for multi-step query refinement, done with \textbf{Qwen2.5 14B Instruct}. Built entirely from \textbf{open-source models}, and can run locally to ensure \textbf{data privacy and eliminate cloud API costs}.}
